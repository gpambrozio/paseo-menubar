...
One icon per status bucket, generated by `scripts/make-icons.mjs` and stored as
template PNGs in `assets/generated/`:

| Bucket | Icon | Source |
| --- | --- | --- |
| `needs_input` | megaphone | lucide-static |
| `failed` | circle-x | lucide-static |
| `attention` | triangle-alert | lucide-static |
| `running` | loader-pinwheel | lucide-static |
| `done` | the Paseo mark | vendored `assets/paseo-logo.svg` |

The tray icon is the icon of the highest-priority non-empty bucket, in
`SECTION_ORDER` (`needs_input` → `failed` → `attention` → `running` → `done`). With
no workspaces at all, the icon is `done`.

`done` gets the app's own mark instead of a sixth borrowed glyph because it is the
resting state: the fleet is quiet, so the tray shows what Paseo is rather than what a
workspace needs. That was the original ask for this icon set, not an afterthought —
an icon that means "nothing needs you" reads better as the app's own identity than as
a state glyph with no state to point at.

The four workspace icons come from `lucide-static` (ISC), pulled at build time so
there is no runtime dependency on it. `stroke="currentColor"` never resolves when
sharp rasterizes a standalone SVG, so the rasterizer substitutes a concrete color
first. Lucide's default `stroke-width="2"` on a 24×24 artboard also reads thin at a
16px menu-bar size, so the rasterizer raises it before rasterizing rather than
upscaling the result afterward — see `scripts/make-icons.mjs` for the chosen weight
and why.

This replaces an earlier three-state scheme (idle/working/attention, drawn as hollow,
half-filled, and filled circles) that predated the workspace-bucket model and never
matched it: it collapsed five buckets down to three home-grown states with no glyph
distinguishing why the icon changed. One icon per bucket removes the second
vocabulary — the icon says exactly what `SECTION_ORDER`'s top entry says, in the same
words as the section headings, which carry the same icon next to their label.

The count is unchanged: workspaces in `needs_input`, `failed`, or `attention`. Not
`done`, because it is the resting state and counting it would badge every quiet
workspace permanently, and not `running`, for the same reason the icon ranks it
last — during a work session something is nearly always running, and badging or
leading with it would mask the state you are actually waiting on. The menu bar is too
narrow for a breakdown; the breakdown is the menu's job.
