...
Builds are Apple Silicon only and need macOS 14 or later. The app and the disk image
...
The app is a Swift package in `PaseoIconPackage/`. What is left at the root is
build tooling under `scripts/`.

```bash
npm install
npm run icons                               # tray glyphs and the app icon
swift run --package-path PaseoIconPackage PaseoIcon
npm run typecheck
npm test                                    # the scripts suite, then swift test
npm run dist -- --version 0.4.0             # build an unsigned bundle
```

The tray glyphs are generated by `npm run icons` and are not committed. The
directory they land in is, empty, because the package declares it as a resource
and will not build without it — a clone that skips the icon step builds and
tests fine, and the app names the missing glyph at launch.
